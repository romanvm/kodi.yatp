...
def _request(data):
    ...
    :return:
    """
    reply = post(json_url, json=data).json()
    try:
        return reply['result']
    except KeyError:
...
def get_path(torrent):
    ...
    while not (progress.iscanceled() or _request({'method': 'check_torrent_added', 'params': None})):
        sleep(1.0)
    if not progress.iscanceled():
        torrent_data = _request({'method': 'get_data_buffer', 'params': None})
        ...
        if videofiles:
            ...
            if index >= 0:
                ...
                while not (progress.iscanceled() or
                               _request({'method': 'check_buffering_complete', 'params': None})):
                    buffer_progress = _request({'method': 'get_data_buffer', 'params': None})
                    torrent_info = _request({'method': 'get_torrent_info',
                                             'params': [torrent_data['info_hash']]})
                ...
                progress.close()
                if not progress.iscanceled():
                    return media_url + torrent_data['files'][videofile[0]].replace('\\', '/')
                else:
                    _request({'method': 'abort_buffering', 'params': None})
                    if (_request({'method': 'get_torrent_info', 'params': [torrent_data['info_hash']]})['progress'] <=
                            __addon__.buffer_size):
                        sleep(0.5)
    ...
    if not (_request({'method': 'check_torrent_added', 'params': None})
            and _request({'method': 'check_buffering_complete', 'params': None})):
